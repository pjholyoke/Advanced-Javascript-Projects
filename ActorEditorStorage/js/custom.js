...
$(function () {
  ...
  if(localStorage.getItem("Actors") != null) {
    var t = JSON.parse(localStorage.getItem("Actors"));

    $.each(t, function(i, v) {
      $('#list').append('<tr><td class="'+i+' name">' + v.fname + ' ' + v.lname + '</td><td class="text-right"><a class="edit"><span class="'+i+'  glyphicon glyphicon-pencil" aria-hidden="true"></span></a><a class="del"><span class="'+i+' glyphicon glyphicon-remove" aria-hidden="true"></span></a></td></td></td></tr>').css('textTransform', 'capitalize');
    });
  ...
  $(document).on('click', '#clr', function (e) {
    ...
    $.bootstrapGrowl("Storage Cleared", { type: 'success' });

    $('#actorList, #action').fadeOut("slow");
  });
...
function addActor() {
  ...
  if(localStorage.getItem("Actors") != undefined)
    temp = JSON.parse(localStorage.getItem("Actors"));
  else
  ...
  localStorage.setItem("Actors", JSON.stringify(temp));
...
function editActor(id) {
  var temp = JSON.parse(localStorage.getItem("Actors"));
  selected = Actors[($(id.target).parent().parent().parent()).index()];
  $(".date-picker").datepicker();
  ...
  $('#editForm').on('click', '#save', function () {
    ...
    // Set temp to Actors, clear storage then rebuild localStorage 'Actors' into JSON from real Actors.
    temp = Actors;
    window.localStorage.clear();
    localStorage.setItem("Actors", JSON.stringify(Actors));

    $(id.target).parent().parent().parent().html('<td class="'+temp.length+' name">' + selected.fname + ' ' + selected.lname + '</td><td class="text-right"><a class="edit"><span class="'+temp.length+' glyphicon glyphicon-pencil" aria-hidden="true"></span></a><a class="del"><span class="'+temp.length+' glyphicon glyphicon-remove" aria-hidden="true"></span></a></td></td></td>');
...
function deleteActor(id) {
  var temp = JSON.parse(localStorage.getItem("Actors"));
  selected = temp[$(id.target).attr('class').split(' ')[0]-1];

  $(id.target).parent().parent().parent().remove();
  temp.splice($(id.target).attr('class').split(' ')[0], 1);
